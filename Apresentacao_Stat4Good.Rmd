...
Formação: Doutorando em Estatística - UFMG
output:
  pdf_document:
    toc: yes
  html_document:
...
t=seq(0,3,by=0.01)
plot(t,acbs(t,0.1,1),main=expression(T%~%BS(alpha,beta==1)),ylab=expression(P(T<=t)==F(t)),xlab="",type='l')
...
#
t=seq(0,3,by=0.01)
plot(t,bs(t,0.1,1),main=expression(T%~%BS(alpha,beta==1)),ylab='Densidade',xlab="",type='l')
...
plot(t,acbs(t,0.1,0.5),main=expression(T%~%BS(alpha==0.1,beta)),ylab=expression(P(T<=t)==F(t)),xlab="",type='l')
...
t=seq(0,3,by=0.01)
plot(t,bs(t,0.1,0.5),main=expression(T%~%BS(alpha==0.1,beta)),ylab='Densidade',xlab="",type='l')
...
```{r, fig.align='center', fig.height=4, fig.width=7, message=FALSE, warning=FALSE, fig.cap='Histograma da Função de Velhossimilhança'}
hist3D(x=alpha, y=beta, z=z, contour=TRUE, facets=TRUE, curtain=F, phi=-30,theta=30)
```

```{r, fig.align='center', fig.height=4, fig.width=7, message=FALSE, warning=FALSE, fig.cap='Grágico de Contorno da Função de Velhossimilhança'}
image(x=alpha, y=beta,z,col = terrain.colors(500))
contour(x=alpha, y=beta, z=z,add=TRUE,levels = pretty(c(-1780,-2000),20))
...
<p style="text-align: justify;">
Outro problema possível é aparecer números muito grandes ou números negativos durante a estimação de forma que o R não consiga continuar o processo de otimização. Veja o menor número positivo que pode ser representado pela máquina, o maior número e outros valores importantes:
</p>
...
par(mfrow=c(1,2))
hist(m[,1],prob=T,ylab="densidade",xlab="",main="");
rug(m[,1])
curve(expr = dnorm(x,mean=mean(m[,1]),sd=sd(m[,1])),add=T, col="red")
hist(m[,2],prob=T,ylab="densidade",xlab="",main="");
rug(m[,2])
...
par(mfrow=c(1,2))
hist(m1[,1],prob=T,ylab="densidade",xlab="",main="");
rug(m1[,1])
curve(expr = dnorm(x,mean=mean(m1[,1]),sd=sd(m1[,1])),add=T, col="red")
hist(m1[,2],prob=T,ylab="densidade",xlab="",main="");
rug(m1[,2])
...
aleatória e com o aumento de beta há um aumento da assimetria e uma diminuição da variância do gráfico.
</p>

```{r, fig.align='center',fig.cap='Função Densidade'}
#
t=seq(0.5,6,by=0.01)
plot(t,bs(t,1,100),main=expression(T%~%BS(mu,phi==100)),ylab='Densidade',xlab="",type='l')
...
main=expression(T%~%BS(alpha,beta==1)),ylab=expression(P(T<=t)==F(t)),xlab="",
...
```{r, fig.align='center',fig.cap='Função Densidade'}
t=seq(0,4,by=0.01)
plot(t,bs(t,1,2),ylim=c(0,2.8),main=expression(T%~%BS(mu==1,phi)),ylab='Densidade',xlab="",type='l')
...
<p style="text-align: justify;">
Ao manter $\mu$ fixo e aumentar $\phi$ a variância de T tende a zero.
</p>
...
plot(phi,V(0.6,phi),ylim=c(0,2.8),main=expression(T%~%BS(mu==1,phi)),ylab='Var(T)',xlab="",type='l')
...
rug(m[,2])
curve(expr = dnorm(x,mean=mean(m[,2]),sd=sd(m[,2])),add=T, col="red")
hist(m[,3],prob=T,ylab="densidade",xlab="",main="");
rug(m[,3])
curve(expr = dnorm(x,mean=mean(m[,3]),sd=sd(m[,3])),add=T, col="red")
hist(m[,4],prob=T,ylab="densidade",xlab="",main="");
rug(m[,4])
...
rug(m1[,2])
curve(expr = dnorm(x,mean=mean(m1[,2]),sd=sd(m1[,2])),add=T, col="red")
hist(m1[,3],prob=T,ylab="densidade",xlab="",main="");
rug(m1[,3])
curve(expr = dnorm(x,mean=mean(m1[,3]),sd=sd(m1[,3])),add=T, col="red")
hist(m1[,4],prob=T,ylab="densidade",xlab="",main="");
rug(m1[,4])
